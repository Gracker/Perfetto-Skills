...
`recorded_populated`、`unknown`。表存在但零行不再被当成“已经录制”。当前声明
fixture 覆盖 API 24、31、32、34、35、36（其中 SmartPerfetto 自有 Trace 为
API 35/36），这仍不是 API 28–37 全覆盖。API 37 没有真实 Trace，因此只能是
...
需要 Python 3.11+、`uv`，以及同级 SmartPerfetto checkout（用于重新导出和真实
Trace 集成测试）：
