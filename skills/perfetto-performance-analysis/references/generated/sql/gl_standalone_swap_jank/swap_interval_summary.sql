...
target AS (
  SELECT COALESCE(
    (SELECT CAST(PERCENTILE(interval_ns, 50) AS INTEGER) FROM swap_intervals),
    16666667
...
SELECT
  ...
  CASE
    ...
    WHEN PERCENTILE(interval_ns, 95) > (SELECT target_interval_ns * 2 FROM target) THEN 'warning'
