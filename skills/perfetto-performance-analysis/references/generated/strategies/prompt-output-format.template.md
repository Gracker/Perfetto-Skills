...
### 通用规则
- `self_ms` / exclusive time 是未被已记录子项覆盖的 wall time，不自动等于机制耗时或可优化收益；父子 slice 不相加。
...
6. 逐句数据引用：覆盖每个可核对命题；重复命题复用 claim/引用。
...
规则：
- 根因句必须自成一条 claim；按实际因果/关系证据引用，不规定引用数量，不借用无关测量值；引用字段和值按原始证据保留。
- 同类指标合并引用，避免重复堆同一张表。
