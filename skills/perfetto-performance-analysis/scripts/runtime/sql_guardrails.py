#!/usr/bin/env python3
"""Analyze authored PerfettoSQL for portable safety invariants and advisories."""
        ...
        if not partition_keys:
            issues.append(
                _issue(
                    ...
                    "span-join-safety",
                    "error",
                    "SPAN_JOIN requires at least one PARTITIONED input so interval matching stays scoped to a stable identity.",
                )
            )
        elif len(partition_keys) > 1 and len(set(partition_keys)) > 1:
            issues.append(
                _issue(
                    sql,
                    issue_index,
